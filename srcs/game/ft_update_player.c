...
{
	player->stats.speed = player->stats.base_speed * (1 + ft_get_dash_status(input) * 1000);
	player->velocity.x = player->stats.speed * (input->keyboard['d'] - input->keyboard['a']);
	player->velocity.y = player->stats.speed * (input->keyboard['s'] - input->keyboard['w']);
